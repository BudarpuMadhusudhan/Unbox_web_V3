...
/* TEST CASE */
describe('Navigate to the Application and verify Uptime', () => {
    it('Login to the application and select a date range and Verify the Uptime displayed', async () => {
        ...
        const result=await dbase.executeQueryInDatabase(testData.dbqueries.sumOfUptimeQuery)
        console.log(result);
        await homepageWorkFlow.compareUptimeWithDB(result)
    })
})
